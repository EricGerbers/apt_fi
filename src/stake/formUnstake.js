...
import { Sol, STSol } from "../../icons/tokens"
...
export const FormUnStake = () => {
  ...
  const {connect, connected, balance} = useAptos()
  const handleSubmit = () => {
    if(amount === 0 || amount === ''){
  ...
  return (
    <div>
      ...
      <Input
        ...
        prefixIcon={<STSol />}
        placeholder='Amount'
        suffix={<Button className='btn-stake-max' disable={!connected} onClick={handleSetMax}>Max</Button>}
      />
      {
      ...
      {
        connected ? (
          <Button className='button-primary btn-form-amount' onClick={handleSubmit}>Submit</Button>
        ) : (
          <Button className='button-primary btn-form-amount' onClick={connect}>Connect Wallet</Button>
        )
      }
      ...
      <div className="form-pool-note">
        <div className='form-pool-note__label'>
          Exchange rate
          <Tooltip
            content={<FormTooltip message='Updates at the end of an epoch. Rising exchange rate indicates an appreciation in stSOL value' />}
            placement="bottom"            
          >
            <i className="bi bi-question-circle"></i>
          </Tooltip>
      ...
      <div className='form-pool-quote'>
        ...
        <div>
          ...
          <p>After that, you can use your wallet (Phantom or Solflare) to withdraw the inactive stake. More info here</p>
        </div>
      </div>
    </div>
  )
