...
export const isConnectedMetaMask = (infoConnect) =>
  infoConnect && infoConnect.connected && infoConnect.address && infoConnect.address !== '';

export const getAddressByRel = (relParam) => {
  let addressByRel = '';
  if (relParam && relParam !== '') {
    //setAddress(relParam);
    let detailRel = listAddress.find((item) => item.relValue === relParam);
    if (detailRel) {
      addressByRel = detailRel.address;
    } else {
      addressByRel = listAddress.find((item) => item.relValue === 'default').address;
    }

    localStorage.setItem('rel', relParam);
  } else {
    let localRel = localStorage.getItem('rel');    
    if (localRel && localRel !== '') {
      let detailRel = listAddress.find((item) => item.relValue === localRel);
      if (detailRel) {
        addressByRel = detailRel.address;
      } else {
        addressByRel = listAddress.find((item) => item.relValue === 'default').address;
...
export const getCurrencyUnit = (chainId) => {
  let formatChainId = parseInt(chainId);
  switch(formatChainId){
    case 56:
    case 97:
      return 'BNB'
    case 1:
